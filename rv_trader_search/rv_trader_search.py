...
Features:
- Polite human jitter delays (7s-15s) and batch resting (30s)
- Automated posted date extraction (JSON-LD, OpenGraph, and CDN timestamps)
- Automatic SQLite migration with days-on-lot triggers, date_posted, and price history
- View creation for real-time market intelligence
- Automated stale inventory delisting
- Discord Webhook integration for price cuts, new listings, and posted dates
...
DB_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "rv_trader_search", "rvtrader_tracker.db")
...
MODELS_CATALOG = [
    {"model": "Gulf Stream Vista Cruiser 17SCD", "key": "17SCD"},
    {"model": "Venture RV Sonic Lite SL160VFB", "key": "160VFB"},
    {"model": "Forest River Rockwood Geo Pro G17SLE", "key": "G17SLE"},
    {"model": "Venture RV Sonic Lite SL150VRK", "key": "150VRK"},
    {"model": "Forest River Rockwood Geo Pro G19FD", "key": "G19FD"},
    {"model": "Forest River Flagstaff E-Pro E19FD", "key": "E19FD"},
    {"model": "Venture RV Sonic Lite SL167VMS", "key": "167VMS"},
    {"model": "Forest River Rockwood Geo Pro G19FBS", "key": "G19FBS"},
    {"model": "Forest River Flagstaff E-Pro E19FBS", "key": "E19FBS"},
    {"model": "Forest River Rockwood Geo Pro G19RL", "key": "G19RL"},
    {"model": "Forest River Flagstaff E-Pro E19RL", "key": "E19RL"},
    {"model": "Venture RV Sonic Lite SL169VMK", "key": "169VMK"},
    {"model": "Forest River Rockwood Geo Pro G17BLE", "key": "G17BLE"},
    {"model": "Venture RV Sonic Lite 169VRK", "key": "169VRK"},
    {"model": "Gulf Stream Vista Cruiser 19CSK", "key": "19CSK"},
    {"model": "Gulf Stream Vista Cruiser 19ERD", "key": "19ERD"},
    {"model": "Forest River Ibex 16MBJ", "key": "16MBJ"},
    {"model": "Forest River Ibex 16MRJ", "key": "16MRJ"},
    {"model": "Coachmen Apex Nano 194BHS", "key": "194BHS"},
    {"model": "Coachmen Apex Nano 187RB", "key": "187RB"},
    {"model": "Coachmen Apex Nano 190RBS", "key": "190RBS"},
    {"model": "Venture RV Sonic Lite SL169VUD", "key": "169VUD"},
    {"model": "Forest River r-pod RP-192", "key": "RP-192"},
    {"model": "Forest River r-pod RP-180", "key": "RP-180"},
    {"model": "Forest River r-pod RP-190", "key": "RP-190"},
    {"model": "Jayco Jay Feather Air 16RB", "key": "16RB"},
...
def build_funtownrv_url(keyword):
    encoded_keyword = urllib.parse.quote_plus(keyword)
    return f"https://www.funtownrv.com/new-rv-sales?s=true&lots=2359%2C2358%2C2357&pagesize=72&sort=year-desc&types=29&q={encoded_keyword}"
...
def sync_unit_to_db(unit):
    ...
    if row is None:
        cursor.execute("""
            INSERT INTO listings (source, source_id, target_model, listing_title, seller_location, 
                                  model_year, condition, current_price, original_price, first_seen, last_seen, 
                                  date_posted, status, url)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', ?)
        """, (
            ...
            unit["price"], unit["price"], now_str, now_str, unit.get("date_posted"), unit["url"]
        ))
        conn.commit()
        conn.close()
        return ("NEW", None)
    ...
    """, (
        ...
        unit.get("condition"), unit.get("seller_location"), unit.get("date_posted"), listing_id
    ))
...
def parse_rvtrader_page(html, target_model, clean_key):
    ...
    for card in cards:
        ...
        if clean_key not in clean_text:
            continue
        link = card.find("a", href=lambda h: bool(h and "/listing/" in h))
        if not link:
            continue
        href = str(link.get("href") or "")
        url = urljoin("https://www.rvtrader.com", href)
        ...
        if not any(u["source_id"] == source_id for u in units):
            units.append({
                ...
                "condition": condition, "price": price, "url": url, "date_posted": posted_date
            })
    return units
...
def parse_rvusa_page(html, target_model, clean_key):
    ...
    for card in cards:
        ...
        if not link:
            continue
        href = str(link.get("href") or "")
        url = urljoin("https://www.rvusa.com", href)
...
def parse_rvt_page(html, target_model, clean_key):
    ...
    for card in cards:
        ...
        if not link:
            continue
        href = str(link.get("href") or "")
        url = urljoin("https://www.rvt.com", href)
...
def parse_truervs_page(html, target_model, clean_key):
    ...
    for card in cards:
        ...
        if not link:
            continue
        href = str(link.get("href") or "")
        url = urljoin("https://truervs.com", href)
...
def parse_rvenvy_page(html, target_model, clean_key):
    ...
    for card in cards:
        ...
        if not link:
            continue
        href = str(link.get("href") or "")
        url = urljoin("https://rvenvy.com", href)
...
def parse_rvuniverse_page(html, target_model, clean_key):
    ...
    for card in cards:
        ...
        if not link:
            continue
        href = str(link.get("href") or "")
        url = urljoin("https://www.rvuniverse.com", href)
...
def parse_autotrader_rv_page(html, target_model, clean_key):
    ...
    for card in cards:
        ...
        if not link:
            continue
        href = str(link.get("href") or "")
        url = urljoin("https://rvs.autotrader.com", href)
        id_match = re.search(r"[-_/]([0-9]{6,10})(?:\.html|\/|$)", url)
        source_id = id_match.group(1) if id_match else f"URL_{abs(hash(url))}"
...
def parse_rvpostings_page(html, target_model, clean_key):
    ...
    if not cards:
        cards = soup.find_all("div", class_=lambda c: bool(c and any(k in str(c).lower() for k in ["listing", "item"])))
    ...
    for card in cards:
        ...
        if not link:
            continue
        href = str(link.get("href") or "")
        url = urljoin("https://www.rvpostings.com", href)
...
def parse_smartrvguide_page(html, target_model, clean_key):
    ...
    if not cards:
        cards = soup.find_all("div", class_=lambda c: bool(c and "listing" in str(c).lower()))
    ...
    for card in cards:
        ...
        if not link:
            continue
        href = str(link.get("href") or "")
        url = urljoin("https://www.smartrvguide.com", href)
...
def parse_funtownrv_page(html, target_model, clean_key):
    soup = BeautifulSoup(html, "html.parser")
    units = []

    cards = soup.select("div[class*='unit-card'], div[class*='inventory-item'], div.rv-card, article")
    if not cards:
        cards = soup.find_all("div", class_=lambda c: bool(c and any(k in str(c).lower() for k in ["unit", "inventory", "vehicle"])))
    ...
    for card in cards:
        ...
        href = str(link.get("href") or "")
        url = urljoin("https://www.funtownrv.com", href)
        if any(bad in url.lower() for bad in ["/account", "/checkout", "/service", "/contact"]):
            continue

        id_match = re.search(r"[-_/]([0-9]{4,10})(?:\.html|\/|$)", url)
        if not id_match:
            stock_match = re.search(r"Stock\s*#\s*([0-9]{4,10})", text, re.I)
            source_id = stock_match.group(1) if stock_match else f"URL_{abs(hash(url))}"
        else:
            source_id = id_match.group(1)

        scrubbed = re.sub(r"(?:msrp|discount|savings?|payments?)\s*:?\s*\$\s*([0-9,]+)", "", text, flags=re.I)
        price = extract_price(scrubbed) or extract_price(text)
        year = extract_year(text)
        if not price or (year and (year < MIN_MODEL_YEAR or year > MAX_MODEL_YEAR)):
            continue
        ...
        loc_match = re.search(r"\b([A-Za-z\s]{3,20},\s*[A-Z]{2})\b", text)
        seller_loc = loc_match.group(1).strip() if loc_match else "Fun Town RV"
        posted_date = extract_posted_date(card)
...
def main():
    ...
    init_db()
    logging.info(f"Aggregator initialized: Checking 10 national portals for {len(MODELS_CATALOG)} models.")
        ...
        for batch_num, chunk in enumerate(model_chunks, 1):
            ...
            for item in chunk:
                ...
                targets = [
                    ...
                    ("FunTownRV", build_funtownrv_url(item["key"]), parse_funtownrv_page)
                ]
